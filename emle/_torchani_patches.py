...
This is a patched version of the original torchani.models module that
calls self.aev_computer using args only to allow use of forward hooks
with TorchScript.
...
class BuiltinModel(torch.nn.Module):
    ...
    def _from_neurochem_resources(
        cls, info_file_path, periodic_table_index=False, model_index=0
    ):
        from torchani import neurochem  # noqa
        ...
        (
            ...
            ensemble_prefix,
            ensemble_size,
        ) = neurochem.parse_neurochem_resources(info_file_path)
        if model_index >= ensemble_size:
            raise ValueError(
                "The ensemble size is only {}, model {} can't be loaded".format(
                    ensemble_size, model_index
                )
            )
        ...
        network_dir = os.path.join(
            "{}{}".format(ensemble_prefix, model_index), "networks"
        )
    ...
    def species_to_tensor(self, *args, **kwargs):
        ...
        # device
        return self._species_to_tensor(*args, **kwargs).to(
            self.aev_computer.ShfR.device
        )
...
class BuiltinEnsemble(BuiltinModel):
    ...
    def members_energies(
        ...
        for nnp in self.neural_networks:
            ...
            shifted_energies = self.energy_shifter(
...
def ANI1x(periodic_table_index=False, model_index=None):
    ...
    if model_index is None:
        return BuiltinEnsemble._from_neurochem_resources(
            info_file, periodic_table_index
        )
    return BuiltinModel._from_neurochem_resources(
        info_file, periodic_table_index, model_index
    )
...
# Patched version of NNPOps.OpimizedTorchANI
