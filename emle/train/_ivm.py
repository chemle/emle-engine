...
class IVM:
    ...
    @staticmethod
    def ivm(aev_0, thr=0.02, n_max=None):
        n_samples = len(aev_0)
        selected = [0]
        ...
        for i in range(1, n_max):
            ...
            K_sel = GPR.sq_aev_kernel(aev_sel, aev_sel)
            K_sel_inv = _torch.linalg.inv(K_sel)
            
            if k_old is None:
                k = GPR.sq_aev_kernel(aev_pen, aev_sel)
            else:
                k_new = GPR.sq_aev_kernel(aev_pen, aev_sel[-1:])
            ...
            k_old = _torch.cat([k[:_torch.argmax(var)], k[_torch.argmax(var)+1:]], dim=0)
    ...
    def perform_ivm(aev_mols, z_mols, atom_ids, species, thr):
        ...
        aev_ivm_allz = [aev_z[ivm_idx_z] 
                        for aev_z, ivm_idx_z in zip(aev_allz, ivm_idx)]
